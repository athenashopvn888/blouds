...
import BloudsWebChat from "./BloudsWebChat";
...
export default function DeliveryContent() {
  ...
  async function handleEmailSubmit(event: React.FormEvent) {
    ...
    try {
      await fetch(`https://script.google.com/macros/s/AKfycbx09_sDal1eMVF1r-hUck4e7oq_XBHEWhGvA79JuhZNQ6P4CdhCas0xE3FfexWQ3hq4/exec?action=delivery_email&email=${encodeURIComponent(email)}&store=BLS01`, { method: "GET", mode: "no-cors" });
    } finally {
  ...
  useEffect(() => {
    ...
    fetch("https://milestone-1-demo.vercel.app/api/catalog?store=BLD", { signal: controller.signal })
  ...
  return <main className={styles.main}>
    ...
    <section className={styles.hero}>
      <Image src="/banners/Blouds_Delivery.webp" alt="Blouds delivery menu" width={1200} height={500} priority sizes="100vw" />
      <div><p>Blouds Dispensary</p><h1>Delivery Menu</h1><span>Browse the shared product catalog. The store confirms current availability and delivery details before an order is accepted.</span></div>
    </section>
    <section className={styles.deliveryDetails} aria-label="Blouds delivery details">
      <strong>$60 PRODUCT MINIMUM</strong>
      <a href="sms:+13656440688"><span>DELIVERY TEXT NUMBER</span> +1 (365) 644-0688</a>
    </section>
    ...
    <section className={styles.howToOrder} aria-labelledby="how-to-order-title">
      <div><p>HOW TO ORDER</p><h2 id="how-to-order-title">Order with the Blouds dispatcher</h2><span>LIVE ORDER connects you with the Blouds dispatcher.</span></div>
      <ol>
        ...
        <li><strong>Confirm with the dispatcher</strong><span>The Blouds dispatcher confirms availability, delivery details, and next steps.</span></li>
      </ol>
    </section>
      ...
      <div className={styles.catalog}>
        ...
        <div className={styles.grid}>{visible.map((product) => <article className={styles.card} key={product.publicProductId}>
          <button type="button" className={styles.imageButton} onClick={() => setSelected(product)} aria-label={`View details for ${product.name}`}>
            {product.images[0] ? <Image src={product.images[0]} alt={`${product.name} on the Blouds delivery menu`} fill sizes="(max-width: 640px) 50vw, 280px" unoptimized /> : <span>Blouds</span>}
          </button>
    ...
    <section className={styles.updates} aria-labelledby="delivery-updates-title">
      <div><p>DELIVERY UPDATES</p><h2 id="delivery-updates-title">Get store delivery updates</h2><span>Enter your email to keep the existing Blouds delivery notification active.</span></div>
    ...
    <div className={styles.ctaSection}><p>Visit us in-store at <strong>117 Queen St W, Brampton</strong> — open <strong>24 hours</strong>. Call <strong>(437) 425-0117</strong>.</p></div>
